...
local BASE_URL = getgenv().TALENTLESS_BASE_URL or "https://raw.githubusercontent.com/cleme/TALENTLESS/main/"
if BASE_URL:sub(-1) ~= "/" then
    BASE_URL = BASE_URL .. "/"
end
getgenv().TALENTLESS_BASE_URL = BASE_URL
...
if gameId == 3929033413 then
    ...
    spoofMidiInfo.MouseButton1Click:Connect(
        function()
            loadstring(fetch("spooferinfo.lua"))()
        end
    )
...
function playbuttonclicked()
    ...
    task.spawn(function()
        ...
        for _, song in pairs(songs) do
            if songFound == false then
                if song.var == true then
                    songscript = fetch("SONGS/" .. song.url)
                songFound = true
                end
            end
        end
    ...
    if iscustom ~= true then -- if its custom u dont need a loader
        if spoofMidiPlz == true then
            -- Spoof MIDI
            loadstring(fetch("midi_spoof_loader.lua"))()
        else
            loadstring(fetch("loader_main.lua"))()
        end
    end
